...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fused_attn:
            from ..backbones.helpers import scaled_dot_product_attention
...
class MambaVision(nn.Module):
    ...
    def _load_state_dict(self,
        ...
        _load_checkpoint(self,
                         pretrained,
                         strict=strict,     # 默认为False，忽略了不匹配的参数
                         )
...
class SegMambaVisionBackbone(MambaVision):
    def __init__(self,
                 ...
                 mlp_ratio,  # MLP 比例
                 num_heads,  # 注意力头数
                 resolution=224,  # 输入图像分辨率
                 drop_path_rate=0.2,  # Drop path 的概率
                 in_chans=3,  # 输入通道数
                 ...
                 layer_scale=None,  # 层级缩放
                 layer_scale_conv=None,  # 层级卷积缩放
                 **kwargs):  # 其他参数
...
def SegBackbone_mamba_vision_T(pretrained=True, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
        ...
        if frozen:
            ...
            for name, param in model.named_parameters():
                if name in state_dict:
                    ...
                    print("{}已冻结".format(name))
                    print(param.requires_grad)
                else:
                    print("{}的参数是否训练——{}".format(name, param.requires_grad))

        # 更改分类头的分类类别数
        #  model.head = nn.Linear(in_features=640, out_features=150, bias=True)
...
@MODELS.register_module()
def SegBackbone_mamba_vision_B(pretrained=True, pretrained_cfg=None, pretrained_cfg_overlay=None, **kwargs):
    ...
    model_path = "../mambavision_base_1k.pth.tar"
    ...
    if pretrained:
        ...
        # 更改分类头的分类类别数
        model.head = nn.Linear(in_features=1024, out_features=150, bias=True)
