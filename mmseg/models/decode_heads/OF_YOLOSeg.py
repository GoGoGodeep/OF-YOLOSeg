...
from C2fPro import C2f_Attention


@MODELS.register_module()
class YoloHead_OFMA_C2fPro(BaseDecodeHead):
    def __init__(self,
                 in_channels,
                 channels,
                 num_classes=2,
        ...
        # 当输入特征选择Feature1时，即接受backbone的输出时，通道数为self.channel_num_1
        # 当输入特征选择Out1时，通道数为self,channel_num_2
        # classes_num即为模型分割结果数
        self.attention = EncoderChannels(self.channel_num_1, classes_num=self.num_classes)
    ...
    def forward(self, features):
        ...
        # 当输入特征选择Feature1时，即接受backbone的输出时，输入x1
        # 当输入特征选择Out1时，输入为p_add_Concat_C2f，即head1
...
@MODELS.register_module()
class YoloHead_C2fPro(BaseDecodeHead):
    def __init__(self,
                 in_channels,
                 channels,
                 num_classes=2,
...
@MODELS.register_module()
class YoloHead_withoutin(BaseDecodeHead):
    """
        无feature1输入
    """
